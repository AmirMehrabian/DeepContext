...
centers_cnn = create_grid_rbf_centers(n1=dim1_bin, n2=dim2_bin, n3=dim3_bin)
...
number_centers = centers_cnn.shape[0]
...
model = model_builder_cnn((dim1_bin, dim2_bin, dim3_bin, 1), output_size=number_actions)
model.summary()

buffer = ReplayBuffer_CNN(capacity=buffer_capacity, input_model_size=[dim1_bin, dim2_bin, dim3_bin, 1],
                          output_model_size=number_actions)
...
for episode_index in range(num_episodes):
    ...
    context = encode_with_rbf(context_uncoded.reshape(1, -1), centers_cnn, gamma).reshape(1, dim1_bin, dim2_bin,
                                                                                          dim3_bin, 1)
    ...
    for counter, step_params in enumerate(np.array(step_list).T):
        ...
        model_input = model_feeder_no_action(context).reshape(1, dim1_bin, dim2_bin, dim3_bin, 1)
        ...
        new_input_sample = model_feeder_no_action(context).reshape(1, dim1_bin, dim2_bin, dim3_bin, 1)
