...
from utils import epsilon_greedy, context_builder, create_grid_rbf_centers, model_builder_cnn, model_builder, \
    model_feeder, encode_with_rbf, ReplayBuffer_CNN, ReplayBuffer, model_feeder_no_action, context_builder_12features
...
for episode_index in range(num_episodes):
    ...
    for counter, step_params in enumerate(np.array((step_list)).T):
        ...
        for index_action, action in enumerate(action_set):
            context = context_builder_12features(r_state, p_jam, p_signal,index_action,config_dict)
        ...
        context = context_builder_12features(r_state, p_jam, p_signal, action_index, config_dict)
